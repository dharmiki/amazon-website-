...
fetch("db.json")
  ...
  .then(products => {
    console.log(products.child)
    const productList = document.getElementById('product-list').innerHTML = prod(products.child);
  })
...
function prod(pro) {
  return pro.map((el, i) => {
    return `
      <a href="childsingle.html?id=${el.id}" class="text-decoration-none text-dark w-[23%] ">
