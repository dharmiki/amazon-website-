...
fetch("db.json")
  ...
  .then(products => {
    ...
    const productList = document.getElementById('product-list').innerHTML = prod(products.men);
  })
...
function prod(pro) {
  return pro.map((el, i) => {
    return `
      <a href="mensingle.html?id=${el.id}" class="text-decoration-none text-dark w-[23%] ">
